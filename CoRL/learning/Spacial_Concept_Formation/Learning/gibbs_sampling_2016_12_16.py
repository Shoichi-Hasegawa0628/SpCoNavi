...
nu_0=hyper_para[2]
mu_0=np.array([map_center_x,map_center_y])
psai_0=np.matrix([[0.05,0.0],[0.0,0.05]])
gamma=hyper_para[3]
beta =hyper_para[4]
image_increment=hyper_para[5] 
word_increment=hyper_para[6]
iteration =int(hyper_para[7]) #00 #iteration num
...
def gibbs(data_pose,data_feature,data_word,word_data_ind):
    ...
    FEATURE_DIM=len(data_feature[0])
    print FEATURE_DIM
    Myu_Ct =np.array([[random.uniform(map_x,MAP_X ),random.uniform(map_y,MAP_Y)] for i in xrange(clas_num)])
    ...
    Sigma_Ct =[np.matrix([[sigma_init,0.0],[0.0,sigma_init]]) for i in range(clas_num)]
    ...
    for iter in xrange(iteration):
        ...
        for d in xrange(0,DATA_NUM,Slide):
            ...
            for i in range(clas_num):
                ...
                prob_C_t[i] +=Multi_prob(data_feature[d],fi_Ct[i])
        ...
        for c in xrange(clas_num):
            ...
            sum_pose=np.array([0.0 for i in range(x_dim)])
            for i in xrange(len(pose_c)):
                for j in xrange(x_dim):
                    sum_pose[j] +=pose_c[i][j]
			
            bar_pose=np.array([0.0 for i in range(x_dim)])
            ...
            if len(pose_c)==0:
                ...
                Sigma_Ct[c]=np.matrix([[sigma_init,0.0],[0.0,sigma_init]])
            #print Myu_Ct[c]
            #print Sigma_Ct[c]
...
if __name__ == '__main__':
    ...
    if CNN_feature==0:
        ...
        feature=np.array(f_r.cnn_feature_data_read(data_diric,image_increment))
    '''
    
    args.feature=data_diric+"/"+args.feature
    print args.feature
    feature=np.array(f_r.feature_data_read(args.feature,image_increment))
    if args.Word:
