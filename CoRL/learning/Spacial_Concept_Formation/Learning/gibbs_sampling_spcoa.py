...
psai_0 = np.matrix([[0.05,0.0,0.0,0.0],[0.0,0.05,0.0,0.0],[0.0,0.0,5.0,0.0],[0.0,0.0,0.0,5.0]])
gamma = hyper_para[5]
...
#======================================Reading Data===========================================
    ...
    for iter in xrange(iteration):
        ...
        all_prob=1.0
        print "iteration"+ repr(iter)+" Spcoa"
        ...
        #<<<<<Sampling class index C_t<<<<
            ...
            for d in xrange(0,data_num[e],Slide):
                r_t=r_t_set[e][d] 
                #*****<2>***** 
                prob_C_t=np.zeros(concept_num)
                
                for i in range(concept_num):
                    prob_C_t[i]+=math.log(G_set[e][i])
                    prob_C_t[i] +=Multi_prob(feature_set[e][d],phi_f[i])
                    prob_C_t[i] +=Multi_prob(function_set[e][d],phi_w[i])
                    prob_C_t[i] +=Multi_prob(name_set[e][d],phi_n[i])
                    prob_C_t[i]+=math.log(pi_set[e][i][r_t])
                ...
                prob_C_t/=sum(prob_C_t)
                C_t =np.random.choice(class_choice,p=prob_C_t)
                C_t_set[e][d]=C_t
                class_count_e[C_t]+= 1.0
                class_region_count[C_t][r_t] +=1.0

               
                print 'Environment:',e,'Iteration:',iter+1,'Data:',d+DATA_initial_index,"prob_r_t",np.max(gauss_prob_set[d]),'r_t',r_t,"prob_C_t",prob_C_t[max_class],'C_t',C_t
            ...
            for c in range(concept_num):
                ...
                for e in range(data_set_num):
                    ...
                    total_feat_e = total_feat_e+((alpha_f_e))
                    ...
                    total_function_e=total_function_e +(alpha_w_e)
                    ...
                    if len(feat_c_e)==0:
                        ...
                        phi_f_e[e][c]=np.array([float(1.0)/FEATURE_DIM for i in range(FEATURE_DIM)])
                        phi_w_e[e][c]=np.array([float(1.0)/FUNCTION_DIM for i in range(FUNCTION_DIM)] )
                        phi_n_e[e][c]=np.array([float(1.0)/NAME_DIM for i in range(NAME_DIM)] )
        ...
        if (iter%1000)==0:
            ...
            for i in range(data_set_num):
                ...
                for r in range(region_num):
                    for c in range(concept_num):
                        ...
                        region_to_function_prob +=np.array(phi_w_e[i][c]*G_set[i][c]*prob)
            ...
            print len(phi_f_e[0])," ",len(phi_w_e[0])," ",len(phi_n_e[0])
