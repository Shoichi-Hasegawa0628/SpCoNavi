...
class AstarPathPlanner():
    ...
    def Sampling_goal(self, Otb_B, THETA):
        ...
        for it in range(K):
            ...
            while (count_it < sample_it[0][it]):
                goal_candidate += [read_data.Map_coordinates_To_Array_index(multivariate_normal.rvs(mean=Myu[it], cov=S[it], size=1, random_state=None))]
                count_it += 1
    ...
    def astar_path_planner(self, THETA, W_index, outputfile, speech_num, start):
        ...
        action_functions = [self.right, self.left, self.up, self.down, self.stay] #, migiue, hidariue, migisita, hidarisita]
        ...
        for gc_index in range(J):
            ...
            while open_list:
                ...
                for act_func, act_cost in zip(action_functions, cost_of_actions):
                    ...
                    q_pev = self.Manhattan_distance(q, goal) * np.log(float(len(action_functions))) #heuristic function
                    q_key = q_cost - q_pev
